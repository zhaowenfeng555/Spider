...
def process_spider():
    ...
    for i in range(777, 778):
        ...
        url = 'http://www.dwanghong.com/ziliao/' + str(i) + '.html'
        soup = fetch_url(url)
        data = soup.find('div', {'class': 'intro-content clearfix'})
        print (data.find('h1').text)

        yy = data.find('div', {'class': 'flower-info'}).find_all('dl')[9].find('dd')
        print (yy.text)
